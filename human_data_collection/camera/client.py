...
class ImageClient:
    def __init__(self, tv_img_shape = None, tv_img_shm_name = None, wrist_img_shape = None, wrist_img_shm_name = None, 
                       image_show = False, server_address = "localhost", port = 5555, Unit_Test = False):
        """
    ...
    def stop(self):
        """External call: stop receive loop (close socket to unblock recv)"""
        self.running = False
    ...
    def _diagnose_connection(self):
        """Output diagnostic info on connection timeout"""
        logger_mp.error(
            f"[Image Client] No data received from image server.\n"
            f"  Server address : tcp://{self._server_address}:{self._port}\n"
            f"  Possible reasons:\n"
            f"    1. Image server is not running on {self._server_address}\n"
            f"    2. Camera initialization failed on the server side\n"
            f"    3. Network unreachable (try: ping {self._server_address})\n"
            f"    4. Port {self._port} is blocked by firewall\n"
            f"    5. Server port mismatch (check server --port argument)"
        )
    ...
    def receive_process(self):
        ...
        self._socket.setsockopt(zmq.RCVTIMEO, 5000)  # 5-second timeout for diagnostics

        print(f"[ImageClient] Connecting to tcp://{self._server_address}:{self._port}")
        self._socket.connect(f"tcp://{self._server_address}:{self._port}")
        self._socket.setsockopt_string(zmq.SUBSCRIBE, "")

        print(f"[ImageClient] Connected successfully, waiting for image data...")
        ...
        try:
            while self.running:
                ...
                if not first_message_received:
                    print(f"[ImageClient] First message received! Size: {len(message)} bytes")
                    self._socket.setsockopt(zmq.RCVTIMEO, -1)  # Remove timeout after first success
...
if __name__ == "__main__":
    ...
    client = ImageClient(image_show = True, server_address='localhost', Unit_Test=False) # deployment test
