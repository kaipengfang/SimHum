...
class _DiTNoiseNet(nn.Module):
    def __init__(
        ...
        # diffusion parameters display
        param_count = sum(p.numel() for p in self.parameters())
        print()
        print("\033[1;34m🧠 Noise Network Initialized:\033[0m")
        print(f"   📊 Diffusion parameters: \033[1;32m{param_count:,}\033[0m ({param_count:.2e})")
        print()
...
class DiffusionTransformerAgent(BaseAgent):
    def __init__(
        ...
        print()
        print("\033[1;32m🤖 Diffusion Transformer Agent Initialized:\033[0m")
        print(f"   🔢 Total trainable parameters: \033[1;32m{total_params:,}\033[0m ({total_params:.2e})")
        print(f"   🎯 Action chunk size: \033[1;32m{self._ac_chunk}\033[0m")
        print(f"   📐 Action dimensions: \033[1;32m{self._ac_dim}\033[0m")
        print(f"   🔄 Training diffusion steps: \033[1;32m{self._train_diffusion_steps}\033[0m")
        print(f"   ⚡ Eval diffusion steps: \033[1;32m{self._eval_diffusion_steps}\033[0m")
        print()
    ...
    def get_robot_only_skip_keys(self):
        """
        Return the list of checkpoint keys to skip in robot_only mode.
        These keys correspond to layers whose shapes are incompatible when loading a 44D checkpoint into a 16D model.
        """
        skip_keys = [
            # Action projection layers
            'noise_net.ac_proj.0.weight',      # [44,44] -> [16,16]
            'noise_net.ac_proj.0.bias',        # [44] -> [16]
            'noise_net.ac_proj.2.weight',      # [hidden,44] -> [hidden,16]
            'noise_net.ac_proj.2.bias',        # [hidden] -> [hidden] (usually same, but skip as a precaution)

            # Final output layers
            'noise_net.eps_out.linear.weight', # [44,hidden] -> [16,hidden]
            'noise_net.eps_out.linear.bias',   # [44] -> [16]

            # Observation processing layers
            '_obs_proc.1.weight',              # [token_dim,44] -> [token_dim,16]
            '_obs_proc.1.bias',                # [token_dim] -> [token_dim] (usually same, but skip as a precaution)
        ]
        return skip_keys

    def detect_checkpoint_action_dim(self, checkpoint_path):
        """
        Detect the action dimension in a checkpoint
        by inspecting the weight shapes of key layers.
        """
        import torch
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        state_dict = checkpoint.get('model', checkpoint)

        # Check the first linear layer of the action projection
        if 'noise_net.ac_proj.0.weight' in state_dict:
            ac_proj_weight = state_dict['noise_net.ac_proj.0.weight']
            detected_ac_dim = ac_proj_weight.shape[0]  # [ac_dim, ac_dim]
            return detected_ac_dim

        # Fallback check: final output layer
        if 'noise_net.eps_out.linear.weight' in state_dict:
            eps_out_weight = state_dict['noise_net.eps_out.linear.weight']
            detected_ac_dim = eps_out_weight.shape[0]  # [ac_dim, hidden_dim]
            return detected_ac_dim

        return None  # Unable to detect

    def should_skip_keys_for_robot_only(self, checkpoint_path):
        """
        Determine whether keys need to be skipped.
        Skipping is only needed when the checkpoint is 44D but the current model is 16D.

        Returns:
            (need_skip: bool, skip_keys: list)
        """
        detected_dim = self.detect_checkpoint_action_dim(checkpoint_path)
        current_dim = self._ac_dim  # Current model's action dimension

        if detected_dim is None:
            print("Warning: Cannot detect checkpoint action dimension")
            return False, []

        print(f"Checkpoint action dimension: {detected_dim}, Current model: {current_dim}")

        if detected_dim != current_dim:
            print(f"Dimension mismatch detected, will skip incompatible layers")
            return True, self.get_robot_only_skip_keys()
        else:
            print(f"Dimensions match, will load all layers normally")
            return False, []
