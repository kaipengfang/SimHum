#!/bin/bash
# Fine-tune: Human-pretrained model -> Robot data
# Usage: ./scripts/finetune_human.sh <buffer_path> <checkpoint_path> [exp_name] [wandb]
...
if [ $# -lt 2 ]; then
    ...
    echo "  checkpoint_path  Path to Human-pretrained checkpoint (.ckpt)"
    echo "  exp_name         Experiment name (default: ft_human)"
...
print_header "Human -> Robot Fine-tuning"
...
print_launch "Starting Human -> Robot fine-tuning..."
echo ""
