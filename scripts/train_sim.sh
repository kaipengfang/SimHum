#!/bin/bash
# Train Sim-only baseline (simulation data only)
# Usage: ./scripts/train_sim.sh <buffer_path> [exp_name] [wandb]

source "$(dirname "$0")/utils.sh"

if [ $# -lt 1 ]; then
    echo "Usage: $0 <buffer_path> [exp_name] [wandb]"
    echo "  buffer_path  Path to sim replay buffer (.pkl)"
    echo "  exp_name     Experiment name (default: sim_only)"
...
print_header "Sim-only Pre-training"
...
print_launch "Starting Sim-only training..."
echo ""
