#!/bin/bash
# Fine-tune: SimHum-pretrained model -> Robot data
# Usage: ./scripts/finetune_simhum.sh <buffer_path> <checkpoint_path> [exp_name] [wandb]
...
if [ $# -lt 2 ]; then
    ...
    echo "  checkpoint_path  Path to SimHum-pretrained checkpoint (.ckpt)"
    echo "  exp_name         Experiment name (default: ft_simhum)"
...
print_header "SimHum -> Robot Fine-tuning (Dual-path)"
...
print_config "Use human adaptor" "true"
print_header ""
...
print_launch "Starting SimHum -> Robot fine-tuning..."
echo ""
...
CUDA_VISIBLE_DEVICES=$gpu_id python finetune.py \
    ...
    wandb.name=${exp_name}_simhum_ft \
