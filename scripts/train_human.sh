#!/bin/bash
# Train Human-only baseline (human motion data only)
# Usage: ./scripts/train_human.sh <buffer_path> [exp_name] [wandb]

source "$(dirname "$0")/utils.sh"

if [ $# -lt 1 ]; then
    echo "Usage: $0 <buffer_path> [exp_name] [wandb]"
    echo "  buffer_path  Path to human replay buffer (.pkl)"
    echo "  exp_name     Experiment name (default: human_only)"
    echo "  wandb        Set 'true' to enable W&B logging (default: false/disabled)"
    exit 1
fi

buffer_path=$1
exp_name=${2:-human_only}
...
print_header "Human-only Pre-training"
...
print_launch "Starting Human-only training..."
echo ""
...
CUDA_VISIBLE_DEVICES=$gpu_id python finetune.py \
    ...
    wandb.name=${exp_name}_human_only \
